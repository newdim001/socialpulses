...
logger = logging.getLogger("socialpulses.oauth.pinterest")
...
class PinterestProvider(OAuthProvider):
    ...
    def get_login_url(self) -> str | dict:
        ...
        if not client_id:
            return {"error": "not_configured", "message": "Pinterest API keys not set"}
        state = str(uuid.uuid4())
        from platform_clients.pinterest import PinterestClient
        url = PinterestClient.get_oauth_authorize_url(client_id, self.redirect_uri, state)
    ...
    async def handle_callback(self, code: str, state: Optional[str] = None) -> OAuthResult:
        ...
        if not client_id or not client_secret:
            raise ValueError("Pinterest API not configured")
        from platform_clients.pinterest import PinterestClient
        try:
            token_data = PinterestClient.exchange_code_for_token(client_id, client_secret, self.redirect_uri, code)
        except Exception as e:
            raise ValueError(f"Pinterest token exchange failed: {e}") from e
        access_token = token_data.get("access_token", "")
        refresh_token = token_data.get("refresh_token")
        client = PinterestClient(access_token, refresh_token)
        try:
            user_info = client.get_user_info()
        except Exception as e:
            raise ValueError(f"Pinterest get_user_info failed: {e}") from e
        return OAuthResult(
    ...
    def refresh_token(self, refresh_token: str) -> Optional[OAuthResult]:
        if not refresh_token:
            return None
        from platform_clients.pinterest import PinterestClient
        client = PinterestClient("", refresh_token)
        try:
