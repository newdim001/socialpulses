...
logger = logging.getLogger("socialpulses.oauth.youtube")
...
class YouTubeProvider(OAuthProvider):
    ...
    def redirect_uri(self) -> str:
        return "https://app.socialpulses.io/api/auth/youtube/callback"

    def get_login_url(self) -> str | dict:
        client_id = os.environ.get("YOUTUBE_CLIENT_ID")
        if not client_id:
            return {"error": "not_configured", "message": "YouTube API keys not set"}
        state = str(uuid.uuid4())
        from platform_clients.youtube import YouTubeClient
        url = YouTubeClient.get_oauth_authorize_url(client_id, self.redirect_uri, state)
    ...
    async def handle_callback(self, code: str, state: Optional[str] = None) -> OAuthResult:
        ...
        from platform_clients.youtube import YouTubeClient
        try:
            token_data = YouTubeClient.exchange_code_for_token(client_id, client_secret, self.redirect_uri, code)
        except Exception as e:
            raise ValueError(f"YouTube token exchange failed: {e}") from e
        access_token = token_data.get("access_token", "")
        refresh_token = token_data.get("refresh_token")
        client = YouTubeClient(access_token, refresh_token)
        try:
            user_info = client.get_user_info()
        except Exception as e:
            raise ValueError(f"YouTube get_user_info failed: {e}") from e
        return OAuthResult(
    ...
    def refresh_token(self, refresh_token: str) -> Optional[OAuthResult]:
        if not refresh_token:
            return None
        from platform_clients.youtube import YouTubeClient
        client = YouTubeClient("", refresh_token)
        try:
