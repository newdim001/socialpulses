...
class LinkedInClient(BasePlatformClient):
    ...
    def exchange_code_for_token(client_id: str, client_secret: str, redirect_uri: str, code: str) -> dict:
        ...
        resp = api_call_with_backoff(lambda: requests.post(LinkedInClient.TOKEN_BASE, data=data))
    ...
    def post_with_media(self, text: str, media_urls: List[str]) -> dict:
        ...
        for url in media_urls:
            register_resp = api_call_with_backoff(lambda: requests.post(
            ...
            api_call_with_backoff(lambda: requests.put(upload_url, data=img_resp.content, headers={"Authorization": f"Bearer {self.access_token}"}))
